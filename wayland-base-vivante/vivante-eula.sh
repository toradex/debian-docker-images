...
# Only prompt for Vivante EULA if Vivante packages are present
dpkg-query -l "*vivante*"  > /dev/null 2>&1
...
EOEULA
        ...
        if [ "$REPLY" != "y" ] && [ "$REPLY" != "Y" ]; then
            echo "Vivante EULA has not been accepted..."
            exit 1
        fi
        ...
        NL=$(awk '/vivante-eula.sh/ {print FNR}' /etc/profile)
        sed -i ${NL%% *}'iexport ACCEPT_FSL_EULA=1' /etc/profile
    else
